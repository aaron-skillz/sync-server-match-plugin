...
local function match_join_attempt(context, dispatcher, tick, state, presence, metadata)
  ...
  print("number of players: " .. state.num_players)
   if state.max_players <= 2 then
...
--   if (tick < 10) then
--     local message = nk.json_encode(hello)
   local hello = "Hello from player: "
    dispatcher.broadcast_message(1, hello)
--     print("tick " .. tick)
    return state
--   else
